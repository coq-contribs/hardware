...
all: $(VOFILES) $(CMOFILES) $(if $(HASNATDYNLINK_OR_EMPTY),$(CMXSFILES)) test\
  ...
  Factorization/Comparator/comp.mli\
...
uninstall_me.sh: Makefile
	echo '#!/bin/sh' > $@ 
	printf 'cd "$${DSTROOT}"$(COQLIBINSTALL)/Hardware && rm -f $(VOFILES) $(VFILES) $(GLOBFILES) $(NATIVEFILES) $(CMOFILES) $(CMIFILES) $(CMAFILES) && find . -type d -and -empty -delete\ncd "$${DSTROOT}"$(COQLIBINSTALL) && find "Hardware" -maxdepth 0 -and -empty -exec rmdir -p \{\} \;\n' >> "$@"
...
clean::
	...
	- rm -rf Factorization/Comparator/comp.mli

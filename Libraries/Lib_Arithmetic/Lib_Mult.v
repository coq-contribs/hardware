...
intros n; simpl in |- *; elim plus_n_O; auto with v62.
Qed.
...
intro.
elim mult_n_O; elim mult_n_O; auto with v62.
intros; simpl in |- *; apply lt_n_S; auto with v62.
Qed.
...
apply lt_mult_cst.
assumption.
apply lt_O with c; auto with v62.
Qed.
...
intros; apply le_lt_plus; auto with v62.
Qed.
...
intros.
rewrite (S_pred m 0); trivial with v62.
...
intros n m; elim (mult_comm m n); auto with v62.
Qed.
...
pattern n at 1 in |- *.
elim mult_1_r.
apply lt_mult_cst; auto with v62.
Qed.
...
rewrite (mult_comm p (n - m)); rewrite (mult_comm p n);
 rewrite (mult_comm p m); auto with v62.
Qed.
...
intros n H_rec H.
right.
elim (plus_eq_zero (n * b) b); auto with v62.
...
simple induction n; simpl in |- *; auto with v62.
Qed.
...
intros n m H.
elim (mult_eq_zero (S n) m); auto with v62.
intro; absurd (S n = 0); auto with v62.
Qed.
...
cut (p * b = 0).
elim (mult_eq_zero p b); auto with v62.
auto with v62.
apply mult_n_O.
intros n H_rec.
simple induction b.
...
clear a; clear b.
intro.
elim (H_rec m); auto with v62.
apply (fun a b : nat => plus_reg_l a b p).
elim (plus_comm (p * n) p).
elim (plus_comm (p * m) p); trivial with v62.
Qed.
...
intros a b p H H1.
elim (mult_reg_l a b p); auto with v62.
intro; absurd (p = 0); auto with v62.
Qed.
...
intros a b pos H.
elim (mult_eq_zero a b H); auto with v62.
intro h.
absurd (a = 0); auto with v62.
Qed. 
...
intros n m H1 H2.
elim H1; elim H2; simpl in |- *; auto with v62.
Qed.
